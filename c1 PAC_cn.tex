...
\begin{itemize}
	...
	\item \textbf{输出}：假设（有时候称为分类器、回归器） $h:\mathcal{X}\rightarrow\mathcal{Y}$.
...
\subsection{受限假设集（归纳偏置）下的最小经验风险准则}
	...
	Start from an ideal case, in which the \textbf{realizability assumption} holds, i.e., there exists $h^*\in\mathcal{H}$, such that $L_{\mathcal{D},f}(h^*) = 0$. It implies that $L_S(h^*)=0$, $L_S(h_S)=0$. However, we are interested in $L_{\mathcal{D},f}(h_S)$.
