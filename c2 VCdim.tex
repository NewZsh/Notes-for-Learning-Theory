...
%
%**Def: VC-dimension**. The VC-dimension of a hypothesis class $\mathcal{H}>, denoted $\text{VCdim}(\mathcal{H})>, is the maximal size of a set $C\subset\mathcal{X}> that can be shattered by $\mathcal{H}>. If $\mathcal{H}> can shatter sets of arbitrarily large size we say that $\mathcal{H}> has infinite VC-dimension.
%
