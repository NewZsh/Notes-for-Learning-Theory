...
Here we does not give a formal proof that $\bm{\Sigma}$ is symmetric but directly using this conclusion, and setting the derivative to zero leads to $\bm{\Sigma}_{\mathrm{ML}} = \sum_{i=1}^m (\bm{x}_i - \bm{\mu}_{\mathrm{ML}})(\bm{x}_i - \bm{\mu}_{\mathrm{ML}})^\top/m $.
	...
	\end{equation*}
Take derivative with regard to $\bm{\eta}$ leas to $\frac{\partial A(\bm{\eta})}{\partial \bm{\eta}} = \sum_{i=1}^m u(\bm{x}_i)/m$, which can in principle be solved to obtain $\bm{\eta}_\mathrm{ML}$.
...
\subsection{Theoretical analysis of MLE}
...
Hence, MLE is equivalent to ERM, \textit{i.e.},
	...
	\end{equation*}
in which the equality holds \textit{iff.} $\mathcal{P}=\mathcal{P}_{\bm{\theta}}$. In some situations, it is easy to prove that MLE guarantees low true risk. For example, consider the problem of estimating the mean of a Gaussian variable of unit variance. Recall that $\bm{\mu}_{\mathrm{ML}} = \frac{1}{m}\sum_{i=1}^m \bm{x}_i$, then

	\begin{equation*}
	\mathop{\mathbb{E}}\limits_{\bm{x}\sim\mathcal{N}(\bm{\mu}, 1)}[l(\bm{\mu}_{\mathrm{ML}}, \bm{x})- l(\bm{\mu}, \bm{x})] = \frac{1}{2} (\bm{\mu}_{\mathrm{ML}}-\bm{\mu})^2
	\end{equation*}

	Also, we want to know the worst case that MLE may achieve. 
...
in which we denote $m_j=\sum_{i=1}^m x_{ij}$. The normalized form of the posterior is then given by $\mathrm{Dir}(\bm{\mu}|\bm{\alpha}+\bm{m})$.
	...
	\begin{equation*}
	p(\lambda|S)\propto \lambda^{a_0-1} \lambda^{N/2} \exp \{-b_0 \lambda - \frac{\lambda}{2}\sum^{i=1}_m (x_i-\mu)^2\}
	\end{equation*}
	
	\textbf{For multi-variate} case, the corresponding prior is Wishart distribution, which is not that important in this notes.
...
\item[Ex3.5] \textbf{Unbiased estimation} (UML Ex24.1) $\theta_{\mathrm{ML}}$, in intrinsic, is a function of observed random variables, and hence has its expectation. If the expectation of an estimation is exactly the parameter in theory, we say that the estimation is unbiased. In the case of exponential family, 
	\begin{equation*} 
