...
$$\mathcal{D}^m(\\{S:L_{\mathcal{D},f}(h_S) > \epsilon\\}) \leq \mathcal{D}^m(M) = \mathcal{D}^m(\mathop{\bigcup} \limits_{h\in\mathcal{H}_B} \\{S: L_S(h)=0\\}) \leq \sum_{h\in\mathcal{H}_B}\mathcal{D}^m(\\{S: L_S(h)=0\\})$$
